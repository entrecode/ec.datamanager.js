      ...
      if (options && options.hasOwnProperty('filter')) {
        for (var key in options.filter) {
          if (options.filter.hasOwnProperty(key)) {
            if (options.filter[key].hasOwnProperty('exact')) {
              query[key] = options.filter[key].exact;
            }
            if (options.filter[key].hasOwnProperty('search')) {
              query[key + '~'] = options.filter[key].search;
            }
            if (options.filter[key].hasOwnProperty('from')) {
              query[key + 'From'] = options.filter[key].from;
            }
            if (options.filter[key].hasOwnProperty('to')) {
              query[key + 'To'] = options.filter[key].to;
            }
            if (options.filter[key].hasOwnProperty('any') && Array.isArray(options.filter[key].any)) {
              query[key] = options.filter[key].any.join(',');
            }
            if (options.filter[key].hasOwnProperty('all') && Array.isArray(options.filter[key].all)) {
              query[key] = options.filter[key].all.join('+');
            }
          }
        }
      }
...
DataManager.prototype.register = function() {
  return this.model('user').createEntry({});
};
