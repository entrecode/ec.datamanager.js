...
DataManager.prototype.getFileURL = function(assetID, locale) {
  var fileURL = this.url.replace('api/' + this.id + '/', 'files/' + assetID);
  fileURL = fileURL + '/url';
      ...
      function(data) {
        ...
        for (var key in body._embedded) {
          if (body._embedded.hasOwnProperty(key)) {
            ...
            listOfModels[modelName] = dataManager.model(modelName);
          }
        }
        return listOfModels;
      });
        ...
        return api.get(url, {
          ...
          if (body.hasOwnProperty('_embedded') && body._embedded.hasOwnProperty((shortID + ':' + modelID))) {
            ...
            return _.map(body._embedded[shortID + ':' + modelID], function(entry) {
              return new Entry(entry, 'Bearer ' + thisDataManager.accessToken, shortID, modelID);
            });
          }
          return [];
        });
    ...
    entryList: function(options) {
      return api.get(url, {
        ...
        if (body.hasOwnProperty('_embedded') && body._embedded.hasOwnProperty((shortID + ':' + modelID))) {
          ...
          return {
            entries: _.map(body._embedded[shortID + ':' + modelID], function(entry) {
              return new Entry(entry, 'Bearer ' + thisDataManager.accessToken, shortID, modelID);
    ...
    return api.get(thisDataManager.assetUrl,
      ...
      function(data) {
        ...
        if (body.hasOwnProperty('_embedded') && body._embedded.hasOwnProperty('ec:api/asset')) {
          ...
          return {
            assets: _.map(body._embedded['ec:api/asset'], function(asset) {
              return new Asset(asset, authHeaderValue);
            }), count: body.count, total: body.total
          };
        }
    ...
    return api.get(thisDataManager.tagUrl,
      ...
      function(data) {
        ...
        if (body.hasOwnProperty('_embedded') && body._embedded.hasOwnProperty('ec:api/tag')) {
          if (!_.isArray(body._embedded['ec:api/tag'])) {
            body._embedded['ec:api/tag'] = [body._embedded['ec:api/tag']];
          }
          return _.map(body._embedded['ec:api/tag'], function(tag) {
            return new Tag(tag, authHeaderValue);
          });
        }
        return [];
      });
