...
DataManager.prototype.getFileURL = function(assetID, locale) {
  ...
  return api.get(fileURL, headers).then(null, function(res) {
    return res.url;
  });
};
...
DataManager.prototype.getImageURL = function(assetID, size, locale) {
  ...
  fileURL = fileURL + '/url';
  var headers = locale ? {'Accept-Language': locale} : null;
  return api.get(fileURL, headers, {size: size, nocrop: true}).then(null, function(res) {
    return res.url;
  });
};
...
DataManager.prototype.getImageThumbURL = function(assetID, size, locale) {
  ...
  var headers = locale ? {'Accept-Language': locale} : null;
  return api.get(fileURL, headers, {size: size, nocrop: false}).then(null, function(res) {
    return res.url;
  });
};
