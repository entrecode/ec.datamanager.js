...
Model.prototype.enableCache = function(maxCacheAge) {
  return this._dm._makeDB().then(function(db) {
    ...
    //this._items.ensureUniqueIndex('_id');
    return this._loadData();
  }.bind(this));
};

Model.prototype._loadData = function() {
...
Model.prototype.entryList = function(options) {
  ...
  if (this._maxAge) {
    ...
    .then(function() {
      switch (options.cacheType || 'default') {
      case 'stale':
        throw new Error('cacheType \'stale\' not implemented.');
      case 'refresh':
        return this._loadData();
      case 'default':
      default:
    ...
    .then(function(items) {
      var res = items.find('getAll');
      var out = [];
      for (var i = 0; i < res.length; i++) {
        out.push(new Entry(halfred.parse(res[i]), this._dm, this));
      }
      return Promise.resolve({ entries: out, count: out.length, total: items.data.length })
    }.bind(this));
  }
