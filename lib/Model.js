...
Model.prototype.enableCache = function(maxCacheAge) {
  return this._dm._makeDB().then(function(db) {
    this._collection = db.getCollection(this.title);
    if (!this._collection) {
      this._collection = db.addCollection(this.title, {
        indices: [
          '_id',
          '_entryTitle'
        ]
...
Model.prototype._loadData = function() {
  return new Promise(function(resolve, reject) {
    this._getTraversal().then(function(traversal) {
      ...
      .get(function(err, res, traversal) {
        util.checkResponse(err, res).then(function(res) {
          ...
          this._collection.removeDataOnly();
          for (var i = 0; i < entries.length; i++) {
            this._collection.insert(entries[i]);
          }
