...
- `fields` - Array of field names to include in the response (Will not work with cached data).
...
returns a Entry object as Promise. Levels property can be used to request nested entries. Fields may be used to only get selected fields in the response (Will not work with cached data). Resolved nested elementes are proper SDK objects with all functions like `save()` and `delete()`.
